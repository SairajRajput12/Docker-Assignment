...
backend_url = os.getenv('BACKEND_URL') # docker wala urls
...
def f_update_project():
    try:
        ...
        response = requests.get(backend_url+"/read_data1")
        data = response.json()
        ...
        payload = {"updated_data": user_data}
        headers = {"Content-Type": "application/json"}

        response = requests.post(backend_url+"/write_data1", json=payload, headers=headers)


        if response.status_code == 200:
            return jsonify({"message": "Project updated successfully!"}), 200
        else: 
            return jsonify({"message":"unable to update the data !"}),501
...
def signup():
    ...
    payload = {"updated_data": users_data}
    headers = {"Content-Type": "application/json"}
...
def add_project(): 
    ...
    if response.status_code == 200:
        print('User data updated successfully.')
        return jsonify({"message": "User added his project succesfully !"}), 200
    else: 
        return jsonify({"message":"unable to update the data !"}),501
